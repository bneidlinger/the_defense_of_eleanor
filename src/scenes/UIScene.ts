...
import {
  CANVAS_W, CANVAS_H, HUD_H, COLORS,
  SCENE_GAME, SCENE_UI,
} from "../config";
...
interface ToolButton { id: string; def: BuildingDef | null; rect: Phaser.GameObjects.Rectangle; label: Phaser.GameObjects.Text; }
...
export class UIScene extends Phaser.Scene {
  ...
  private readonly y0 = CANVAS_H - HUD_H; // top of the HUD strip
  ...
  override update(): void {
    ...
    for (const b of this.buttons) {
      ...
      const afford = b.def ? canAfford(gs.economy, b.def.cost) : true;
      b.label.setColor(afford ? hex(COLORS.text) : hex(COLORS.danger));
    }
